...
vec3 cheap_reflect(vec3 ori, vec3 dir) {
    vec3 L = lightDiffuseColor;
    if (rayBoxIntersect(ori + vec3(-278, -548.8, -292), dir, lightBound))
    {
        L *= lightColor;
    }
    // TODO: walls
...
vec3 render(vec3 ray_start, vec3 ray_dir) {
    ...
    if (p.z >= 0.) {
        if (objectID == OBJ_WATER) {
            ...
            color = fresnel * cheap_reflect(newOri, R) + (1. - fresnel) * refractColor;
