...
const vec3 waterBlueColor = vec3(0.00, 0.52, 0.78);
...
vec3 normal(vec2 pos, float e, float depth) {
    vec2 ex = vec2(e, 0);
    H = getWaves(pos.xy * 0.1, WAVE_NORMAL_COMPLEXITY) * depth;
    vec3 a = vec3(pos.x, H, pos.y);
    return normalize(cross(
        normalize(a - vec3(pos.x - e, getWaves(pos.xy * 0.1 - ex.xy * 0.1, WAVE_NORMAL_COMPLEXITY) * depth, pos.y)),
        normalize(a - vec3(pos.x, getWaves(pos.xy * 0.1 + ex.yx * 0.1, WAVE_NORMAL_COMPLEXITY) * depth, pos.y + e))));
}
    ...
    if (p.z >= 0.) {
        if (objectID == OBJ_WATER) {
            ...
            */
            ...
            if (_waterInte)
                // color = mix(color, waterBlueColor, WATER_BLUE_MIX);
